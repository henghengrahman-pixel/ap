...
router.get('/predictions', (req, res) => res.json({ success: true, predictions: getPredictions() }));
router.get('/news', (req, res) => res.json({ success: true, news: readJson('news.json', []).filter(n => n.active !== false) }));
...
router.get('/home-data', (req, res) => res.json({ success: true, settings: getSettings(true), messages: readJson('messages.json', []), analytics: readJson('analytics.json', { clicks: {}, installs: 0 }), predictions: getPredictions() }));
...
router.post('/push/token', (req, res) => {
  ...
  saveToken(token); subscribeTopic(token, 'all'); res.json({ success: true });
});
